...
def _es_name_exact_phrase(literal):
    ...
    return Q(
        {
            "match_phrase": {
                "display_name.exact": {"query": literal, "analyzer": "exact_analyzer"}
            }
        }
    )
...
def _parse_es_search_terms(search, search_terms):
    ...
    def must_group_matcher(match):
        ...
        group_query = Q(
            "bool", should=[_es_name_exact_phrase(lit_m[1]) for lit_m in literals]
        )
    ...
    parsed_search_terms = QUOTED_LITERAL_GROUP_REGEX.sub(
        must_group_matcher, search_terms
    ).strip()
    parsed_search_terms = QUOTED_LITERAL_REGEX.sub(
        must_matcher, parsed_search_terms
    ).strip()

    # Create phrase matches (if any)
    must_queries = [_es_name_exact_phrase(lit) for lit in must_set] + must_or_groups
    must_not_queries = [
        _es_name_exact_phrase(lit) for lit in must_not_set
    ] + must_not_or_groups
    ...
    if must_queries or must_not_queries:
        ...
        combined_search_query = Q("bool", must=must_queries, must_not=must_not_queries)
...
def search_elastic(
    ...
    s = Search(
        using=es_client, index=app.config.get("ES_INDEX_NAME")
    )  # todo, sukebei prefix
...
def search_db(
    ...
    if term:
        for item in shlex.split(term, posix=False):
            if len(item) >= 2:
                qpc.filter(
                    FullTextSearch(item, models.TorrentNameSearch, FullTextMode.NATURAL)
                )
    ...
    if sort_column.class_ != models.Torrent:
        ...
        query = query.with_hint(
            sort_column.class_, "USE INDEX ({0})".format(index_name)
        )
    ...
    if rss:
        ...
        query = query.paginate_faste(
            page, per_page=per_page, step=5, count_query=count_query, max_page=MAX_PAGES
        )
...
BAKED_SORT_LAMBDAS = {
    ...
    "seeders-asc": lambda q: q.join(models.Statistic)
    .with_hint(models.Statistic, "USE INDEX (idx_nyaa_statistics_seed_count)")
    .order_by(models.Statistic.seed_count.asc(), models.Torrent.id.asc()),
    "seeders-desc": lambda q: q.join(models.Statistic)
    .with_hint(models.Statistic, "USE INDEX (idx_nyaa_statistics_seed_count)")
    .order_by(models.Statistic.seed_count.desc(), models.Torrent.id.desc()),
    "leechers-asc": lambda q: q.join(models.Statistic)
    .with_hint(models.Statistic, "USE INDEX (idx_nyaa_statistics_leech_count)")
    .order_by(models.Statistic.leech_count.asc(), models.Torrent.id.asc()),
    "leechers-desc": lambda q: q.join(models.Statistic)
    .with_hint(models.Statistic, "USE INDEX (idx_nyaa_statistics_leech_count)")
    .order_by(models.Statistic.leech_count.desc(), models.Torrent.id.desc()),
    "downloads-asc": lambda q: q.join(models.Statistic)
    .with_hint(models.Statistic, "USE INDEX (idx_nyaa_statistics_download_count)")
    .order_by(models.Statistic.download_count.asc(), models.Torrent.id.asc()),
    "downloads-desc": lambda q: q.join(models.Statistic)
    .with_hint(models.Statistic, "USE INDEX (idx_nyaa_statistics_download_count)")
...
BAKED_FILTER_LAMBDAS = {
    ...
    "1": lambda q: (
        q.filter(
            models.Torrent.flags.op("&")(models.TorrentFlags.REMAKE.value).is_(False)
        )
    ),
    "2": lambda q: (
        q.filter(
            models.Torrent.flags.op("&")(models.TorrentFlags.TRUSTED.value).is_(True)
        )
    ),
    "3": lambda q: (
        q.filter(
            models.Torrent.flags.op("&")(models.TorrentFlags.COMPLETE.value).is_(True)
        )
    ),
}
...
def search_db_baked(
    ...
    if term:
        ...
        count_query = bakery(
            lambda session: session.query(sqlalchemy.func.count(models.Torrent.id))
        )
            ...
            if not same_user or rss:
                qpc += lambda q: (
                    q.filter(
                        models.Torrent.flags.op("&")(
                            int(
                                models.TorrentFlags.HIDDEN
                                | models.TorrentFlags.ANONYMOUS
                            )
                        ).is_(False)
                    )
                )
            ...
            if overflow > 0:
                ...
                removed_keys = [
                    ...
                    for key, value in sorted(
                        self.entries.items(), key=lambda t: t[1][1]
                    )
                ][:overflow]
...
def baked_paginate(
    query, count_query, params, page=1, per_page=50, max_page=None, step=5
):
    if page < 1:
        flask.abort(404)
    ...
    if app.config["COUNT_CACHE_DURATION"]:
        ...
        if total_query_count is None:
            ...
            LRU_CACHE.put(
                query_key, total_query_count, expiry=app.config["COUNT_CACHE_DURATION"]
            )
    else:
        total_query_count = count_query(ses).params(**params).scalar()
