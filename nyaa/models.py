...
class TorrentBase(DeclarativeHelperBase):
    ...
    def user(cls):
        return db.relationship(
            "User", uselist=False, back_populates=cls._table_prefix("torrents")
        )

    @declarative.declared_attr
    def main_category(cls):
        return db.relationship(
            cls._flavor_prefix("MainCategory"),
            uselist=False,
    ...
    def sub_category(cls):
        ...
        return db.relationship(
            cls._flavor_prefix("SubCategory"),
            uselist=False,
            backref="torrents",
            lazy="joined",
    ...
    def filelist(cls):
        return db.relationship(
            cls._flavor_prefix("TorrentFilelist"),
            uselist=False,
    ...
    def comments(cls):
        return db.relationship(
            cls._flavor_prefix("Comment"), uselist=True, cascade="all, delete-orphan"
        )
...
class TorrentFilelistBase(DeclarativeHelperBase):
    ...
    def torrent(cls):
        return db.relationship(
            cls._flavor_prefix("Torrent"), uselist=False, back_populates="filelist"
        )
...
class StatisticBase(DeclarativeHelperBase):
    ...
    def torrent(cls):
        return db.relationship(
            cls._flavor_prefix("Torrent"), uselist=False, back_populates="stats"
        )
...
class MainCategoryBase(DeclarativeHelperBase):
    ...
    def sub_categories(cls):
        return db.relationship(
            cls._flavor_prefix("SubCategory"), back_populates="main_category"
        )

    @declarative.declared_attr
    def torrents(cls):
        return db.relationship(
            cls._flavor_prefix("Torrent"), back_populates="main_category"
        )
...
class CommentBase(DeclarativeHelperBase):
    ...
    def torrent_id(cls):
        return db.Column(
            db.Integer,
            db.ForeignKey(cls._table_prefix("torrents.id"), ondelete="CASCADE"),
            nullable=False,
        )
    ...
    def torrent(cls):
        return db.relationship(
            cls._flavor_prefix("Torrent"), uselist=False, back_populates="comments"
        )
...
class User(db.Model):
    ...
    nyaa_torrents = db.relationship(
        "NyaaTorrent", back_populates="user", lazy="dynamic"
    )
    nyaa_comments = db.relationship(
        "NyaaComment", back_populates="user", lazy="dynamic"
    )

    sukebei_torrents = db.relationship(
        "SukebeiTorrent", back_populates="user", lazy="dynamic"
    )
    sukebei_comments = db.relationship(
        "SukebeiComment", back_populates="user", lazy="dynamic"
    )
    ...
    def satisfies_trusted_reqs(self):
        num_total = 0
        downloads_total = 0
        for ts_flavor, t_flavor in (
            (NyaaStatistic, NyaaTorrent),
...
class ReportBase(DeclarativeHelperBase):
    ...
    def torrent(cls):
        return db.relationship(
            cls._flavor_prefix("Torrent"), uselist=False, lazy="joined"
        )
...
class TrustedReview(db.Model):
    ...
    application = db.relationship(
        "TrustedApplication", uselist=False, lazy="joined", foreign_keys=[app_id]
    )
