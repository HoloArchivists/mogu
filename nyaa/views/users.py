...
def view_user(user_name):
    ...
    if flask.request.method == "POST" and ban_form and doban and ban_form.validate():
        ...
        flask.flash(
            flask.Markup("User has been successfully {0}.".format(action)), "success"
        )
        return flask.redirect(url)
    ...
    if use_elastic and search_term:
        ...
        max_search_results = app.config.get(
            "ES_MAX_SEARCH_RESULT", DEFAULT_MAX_SEARCH_RESULT
        )

        # Only allow up to (max_search_results / page) pages
        max_page = min(
            query_args["page"], int(math.ceil(max_search_results / results_per_page))
        )
...
def view_user_comments(user_name):
    ...
    comments_query = comments_query.paginate_faste(
        page_number, per_page=comments_per_page, step=5
    )
    return flask.render_template(
...
def activate_user(payload):
    if app.config["MAINTENANCE_MODE"]:
        flask.flash(
            flask.Markup("<strong>Activations are currently disabled.</strong>"),
            "danger",
        )
...
def nuke_user_torrents(user_name):
    ...
    for log_flavour, num in (
        (models.NyaaAdminLog, nyaa_banned),
...
def nuke_user_comments(user_name):
    ...
    for log_flavour, num in (
        (models.NyaaAdminLog, nyaa_deleted),
