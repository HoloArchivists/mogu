...
def home(rss):
    ...
    if infohash_torrent:
        ...
        flask.flash(
            flask.Markup(
                "You were redirected here because "
                "the given hash matched this torrent."
            ),
            "info",
        )
    ...
    if use_elastic and search_term:
        ...
        if render_as_rss:
            ...
            max_results = min(
                max_search_results, query_results["hits"]["total"]["value"]
            )
        ...
        if render_as_rss:
            return render_rss(
                "Home", query, use_elastic=False, magnet_links=use_magnet_links
            )
        else:
            rss_query_string = _generate_query_string(
                search_term, category, quality_filter, user_name
