...
environment_epillog = (
    "You may also provide environment variables NYAA_API_HOST, NYAA_API_USERNAME"
...
conn_group.add_argument(
    ...
    default=False,
    action="store_true",
    help="Upload to sukebei.nyaa.si",
)

conn_group.add_argument("-u", "--user", help="Username or email")
conn_group.add_argument("-p", "--password", help="Password")
conn_group.add_argument(
    "--host", help="Select another api host (for debugging purposes)"
)

resp_group = parser.add_argument_group("Response options")

resp_group.add_argument(
    "--raw", default=False, action="store_true", help="Print only raw response (JSON)"
)
resp_group.add_argument(
    "-m", "--magnet", default=False, action="store_true", help="Print magnet uri"
)

tor_group = parser.add_argument_group("Torrent options")

tor_group.add_argument(
    "-c", "--category", required=True, help="Torrent category (see ). Required."
)
...
tor_group.add_argument(
    "-d", "--description", help="Description for the torrent (optional)"
)
tor_group.add_argument(
    "-D",
    "--description-file",
...
tor_group.add_argument(
    ...
    default=False,
    action="store_true",
    help="Upload torrent anonymously",
)
tor_group.add_argument(
    "-H",
    "--hidden",
...
trusted_group.add_argument(
    ...
    dest="trusted",
    action="store_false",
    help="Do not mark torrent as trusted",
)
parser.set_defaults(trusted=True)

tor_group.add_argument(
    "torrent", metavar="TORRENT_FILE", help="The .torrent file to upload"
)
...
if __name__ == "__main__":
    ...
    if not crude_torrent_check(torrent_file):
        raise Exception(
            "File '{}' doesn't seem to be a torrent file".format(args.torrent)
        )
    ...
    if not (api_username and api_password):
        raise Exception(
            "No authorization found from arguments or environment variables."
        )
