...
@click.option(
    "--temp/--no-temp",
    help="Mark this entry as one that may be " "cleaned out occasionally.",
    default=False,
)
...
def ban(temp, cidrrange):
    if not is_cidr_valid(cidrrange):
        click.secho(
            "{} is not of the format xxx.xxx.xxx.xxx/xx.".format(cidrrange),
            err=True,
            fg="red",
        )
        sys.exit(1)
    with app.app_context():
        ban = models.RangeBan(
            cidr_string=cidrrange, temp=datetime.utcnow() if temp else None
        )
        db.session.add(ban)
        db.session.commit()
