...
class TestBencode(unittest.TestCase):
    ...
    def test_encode(self):
        ...
        test_cases = [  # (raw, expected_result)
            ...
            (
                ...
                b"d5:hello5:world7:numbersli1ei2eee",
            ),  # dict
        ]

        for raw, raised_exception, expected_result_regexp in exception_test_cases:
            self.assertRaisesRegexp(
                raised_exception, expected_result_regexp, bencode.encode, raw
            )
    ...
    def test_decode(self):
        exception_test_cases = [  # (raw, raised_exception, expected_result_regexp)
            ...
            (
                b"l4:hey",
                bencode.MalformedBencodeException,
                r"Read only \d+ bytes, \d+ wanted",
            ),
            (b"ie", bencode.MalformedBencodeException, r"Unable to parse int"),
            (b"i64", bencode.MalformedBencodeException, r"EOF, expecting more integer"),
            (b"", bencode.MalformedBencodeException, r"EOF, expecting kind"),
            (
                b"i6-4",
                bencode.MalformedBencodeException,
            ...
            (b"$:string", bencode.MalformedBencodeException, r"Unexpected data type"),
            (
                b"d5:world7:numbersli1ei2eee",
                bencode.MalformedBencodeException,
        ...
        test_cases = [  # (raw, expected_result)
            ...
            (
                b"d5:hello5:world7:numbersli1ei2eee",
                {"hello": b"world", "numbers": [1, 2]},
            ),  # dict
        ]

        for raw, raised_exception, expected_result_regexp in exception_test_cases:
            self.assertRaisesRegexp(
                raised_exception, expected_result_regexp, bencode.decode, raw
            )
