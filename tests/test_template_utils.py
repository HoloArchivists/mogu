...
from nyaa.template_utils import (
    _jinja2_filter_rfc822,
    _jinja2_filter_rfc822_es,
    get_utc_timestamp,
    get_display_time,
    timesince,
    filter_truthy,
...
class TestTemplateUtils(NyaaTestCase):
    ...
    def test_filter_rfc822(self):
        ...
        test_date = datetime.datetime(
            2017, 2, 15, 11, 15, 34, 100, datetime.timezone.utc
        )
        self.assertEqual(
            _jinja2_filter_rfc822(test_date), "Wed, 15 Feb 2017 11:15:34 -0000"
        )
    ...
    def test_filter_rfc822_es(self):
        ...
        expected = formatdate(
            float(datetime.datetime(2017, 2, 15, 11, 15, 34, 100).timestamp())
        )
    ...
    def test_timesince(self):
        ...
        self.assertEqual(
            timesince(now - datetime.timedelta(seconds=5)), "5 seconds ago"
        )
        self.assertEqual(timesince(now - datetime.timedelta(minutes=1)), "1 minute ago")
        self.assertEqual(
            timesince(now - datetime.timedelta(minutes=38, seconds=43)),
            "38 minutes ago",
        )
        self.assertEqual(
            timesince(now - datetime.timedelta(hours=2, minutes=38, seconds=51)),
            "2 hours ago",
        )
